...
if ($company) {	
	...
	if ($ParentAsset) {
		...
		$date = Get-Date 
		$allhotfixesInstalled = get-hotfix
		...
		$updatesHTML = ($allhotfixesInstalled | select-object InstalledOn, Hotfixid, caption, InstalledBy  | convertto-html -Fragment | out-string) -replace $TableStyling
